...
public class KabbadiDashBoardFragment extends BaseFragment implements IOnItemGamesClickListener, IHTHBattleView {
    @BindView(R.id.rv_kabaadi_games)
    RecyclerView rvKabaadiGames;
...
//        gamesDataArrayList = getData();
        ...
        rvKabaadiGames.setHasFixedSize(true);
...
//        getData();
    ...
    public void onGamesItemClick(View view, int position, int tag) {
        if (mMatchList.get(position).isLive()) {
            ...
            i.putExtra(AppConstant.FROM, AppConstant.FROM_KABAADI);
