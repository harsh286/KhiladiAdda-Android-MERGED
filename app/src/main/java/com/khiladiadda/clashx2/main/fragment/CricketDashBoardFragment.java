...
public class CricketDashBoardFragment extends BaseFragment implements IOnItemGamesClickListener, IHTHBattleView {
    @BindView(R.id.rv_cricket_games)
    RecyclerView rvCricketGames;
...
//        gamesDataArrayList = getData();
        ...
        rvCricketGames.setHasFixedSize(true);
...
//            Snackbar.make(mBackIV, R.string.error_internet, Snackbar.LENGTH_SHORT).show();
    ...
    public void onGamesItemClick(View view, int position, int tag) {
        if (mMatchList.get(position).isLive()) {
            ...
            i.putExtra(AppConstant.FROM, AppConstant.FROM_CRICKET);
