...
public class SelectedPlayers extends BaseActivity implements IHTHBattleView, ICreateBattleView {
    ...
    TextView mContestId;
    @BindView(R.id.cl_football_editable)
    ConstraintLayout mFootballEditable;
    @BindView(R.id.cl_cricket_editable)
    ConstraintLayout mCricketEditable;
    @BindView(R.id.cl_football_below)
    ConstraintLayout mFootballBelow;
    @BindView(R.id.cl_cricket_below)
    ...
    protected void initVariables() {
        ...
        if (mMatch_Type == 2) {
            mFootballEditable.setVisibility(View.VISIBLE);
            mFootballBelow.setVisibility(View.VISIBLE);
            mCricketEditable.setVisibility(View.GONE);
            mCricketBelow.setVisibility(View.GONE);
            mTeamBackground.setVisibility(View.INVISIBLE);
            mTeamBackgroundFootball.setVisibility(View.VISIBLE);
        } else {
            mFootballEditable.setVisibility(View.GONE);
            mFootballBelow.setVisibility(View.GONE);
            mCricketEditable.setVisibility(View.VISIBLE);
            mCricketBelow.setVisibility(View.VISIBLE);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_cancel:
                //change dialog from acceptAlert to cancel dialog
                acceptAlert(this);
                break;
            case R.id.editable:
    ...
    public void onCancelBattle(HTHCancelResponse responseModel) {
        if (responseModel.isStatus()) {
            ...
            AppUtilityMethods.showMsgNew(this, this, "Battle Cancelled Successfully", true, true);
//            finish();
        }
    }
    ...
    private void setData(List<BattlesDeatilsHTH> mMyBattleList) {
        ...
        if (mMyBattleList.get(0).getCaptainId().equalsIgnoreCase(mAppPreference.getProfileData().getId())) {
            ...
            if (mBattleOpponentDetails != null && !mBattleOpponentDetails.isEmpty()) {
                ...
                mOppTV.setText("Opponent's Combo");
...
//                setResult(RESULT_OK, resultIntent);
                finish();
                Intent i = new Intent(SelectedPlayers.this, MyFanLeagueActivityHTH.class);
