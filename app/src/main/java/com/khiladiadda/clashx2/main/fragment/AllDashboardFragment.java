...
public class AllDashboardFragment extends BaseFragment implements IOnItemGamesClickListener, IHTHBattleView {
    @BindView(R.id.rv_all_games)
...
//        gamesDataArrayList = getData();
        ...
        rvAllGames.setHasFixedSize(true);
        rvAllGames.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false));
        rvAllGames.setAdapter(gamesDashBoardAdapter);
...
//            Snackbar.make(mBackIV, R.string.error_internet, Snackbar.LENGTH_SHORT).show();
    ...
    public void onGamesItemClick(View view, int position, int tag) {
        if (mMatchList.get(position).isLive()) {
            ...
            i.putExtra(AppConstant.FROM, mMatchList.get(position).getMatch_type());
            Log.e("TAG", "onGamesItemClick: " + mMatchList.get(position).getMatch_type());
