...
public class LudoUniAdapter extends RecyclerView.Adapter<LudoUniAdapter.LudoContestHolder> {

    private Context mContext;
    private List<LudoContest> mLudoChallengeList;
    private IOnItemChildClickListener mOnItemChildClickListener;

    public LudoUniAdapter(Context context, List<LudoContest> ludoChallengeList) {
    ...
    public void onBindViewHolder(LudoContestHolder holder, int position) {
        ...
        if (ludoContestBean.getContestStatus() != 1) {
            ...
            holder.mAcceptTV.setBackgroundResource(R.color.redhth);
            holder.mAcceptTV.setTextColor(mContext.getResources().getColor(R.color.white));
            if (mIsCaptain) {
        ...
        public void onClick(View v) {
            if (v.getId() == R.id.tv_accept) {
                if (mOnItemChildClickListener != null) {
                    mOnItemChildClickListener.onAcceptClicked(getAdapterPosition());
                }
            }
        }
    }
