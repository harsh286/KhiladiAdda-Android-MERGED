...
public class QuizQuestionImageFragment extends BaseFragment {
    ...
    @BindView(R.id.iv_quiz_image) ImageView mQuestionIV;
    ...
    @Override protected void initBundle(Bundle bundle) {
        ...
        mQuizId = bundle.getString(AppConstant.EXTRA_QUIZ_ID);
    }

    @Override protected void initVariables() {
        String image = mQuestionDetails.getImage();
        if (!TextUtils.isEmpty(image)) {
            Bitmap bmp = ImageUtils.getProfileImageBitmap(AppConstant.APP_DIRECTORY_PATH + File.separator + mQuizId + "_" + mQuestionDetails.getId() + ImageUtils.getExtension(mQuestionDetails.getImage()), null, 200);
            mQuestionIV.setImageBitmap(bmp);
        } else {
            mQuestionIV.setVisibility(View.GONE);
        }
