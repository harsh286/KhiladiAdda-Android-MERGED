...
public class LeaderboardInteractor {
    ...
    public Subscription getLeaderBoardLudoTournament(IApiListener<LeaderboardMainResponse> listener, int page, int limit, String type) {
        ApiManager manager = ApiManager.getInstance();
        ApiService service = manager.createService();
        return manager.createObservable(service.getLeaderBoardLudoTournament(type, page, limit)).subscribe(new SubscriberCallback<>(listener));
    }

    public Subscription getLeaderBoardCallBreak(IApiListener<LeaderboardMainResponse> listener, int page, int limit, String type) {
        ApiManager manager = ApiManager.getInstance();
        ApiService service = manager.createService();
        return manager.createObservable(service.getLeaderBoardCallBreak(type, page, limit)).subscribe(new SubscriberCallback<>(listener));
    }

    public Subscription getLeaderBoardRummy(IApiListener<LeaderboardMainResponse> listener, int page, int limit, String type) {
        ApiManager manager = ApiManager.getInstance();
        ApiService service = manager.createService();
        return manager.createObservable(service.getLeaderBoardRummy(type, page, limit)).subscribe(new SubscriberCallback<>(listener));
    }
}